...
sudo apt-get install steam -y >> ~/logHoje
 
echo "Instalando o Wine"
udo apt-get install wine winetricks wine-mono wine_gecko vulkan-icd-loader lib32-vulkan-icd-loader vkd3d lib32-vkd3d gvfs -y >> ~/logHoje

echo "Iniciando a instalção do Lutris"
#sudo apt-get install lutris python-magic >> log

echo "Instalando dependencias adicionais"
sudo apt-get install gvfs:i386 wine32-preloader:i386 wine64-preloader wine-binfmt gstreamer1.0-libav:i386 gstreamer1.0-plugins-bad:i386 gstreamer1.0-plugins-ugly winetricks -y >> ~/logHoje
...
sudo apt-get install mingw-w64 flvmeta smpeg-plaympeg lame mjpegtools x265 x264 nvidia-vdpau-driver mpv mpg123 libxvidcore4 fluidsynth -y >> ~/logHoje
